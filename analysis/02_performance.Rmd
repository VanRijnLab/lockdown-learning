...
```{r}
change_sd <- sd(prop_change_window$prop_change) * 100 

ggplot(progress_lockdown[school_year == "19/20" & course == "French"], aes(x = chapter_simple, y = (prop_change * 100), colour = chapter_simple, group = school_year)) +
  geom_rect(xmin = 0, xmax = 1000, ymin = -2*change_sd, ymax = 2*change_sd, fill = "grey90", colour = NA) +
  geom_rect(xmin = 0, xmax = 1000, ymin = -change_sd, ymax = change_sd, fill = "grey75", colour = NA) +
  facet_grid(level ~ year, scales = "free_x") +
  geom_hline(yintercept = 0, lty = 2) +
  geom_segment(aes(xend = chapter_simple), yend = 0) +
  geom_point() +
...
ggsave("../output/progress_change_french.pdf", width = 5, height = 5)
ggsave("../output/progress_change_french.png", width = 5, height = 5)
```
...
```{r}
ggplot(progress_lockdown[course == "English"], aes(x = school_year, y = prop, fill = chapter_simple, group = school_year)) +
  facet_grid(level ~ year) +
  geom_col(colour = NA) +
  scale_y_continuous(labels = scales::percent_format(), limits = c(0,1)) +
  scale_fill_viridis_d(direction = -1, na.translate = FALSE) +
...
```{r}
ggplot(progress_lockdown[school_year == "19/20" & course == "English"], aes(x = chapter_simple, y = (prop_change * 100), colour = chapter_simple, group = school_year)) +
  geom_rect(xmin = 0, xmax = 1000, ymin = -2*change_sd, ymax = 2*change_sd, fill = "grey80", colour = NA) +
  geom_rect(xmin = 0, xmax = 1000, ymin = -change_sd, ymax = change_sd, fill = "grey65", colour = NA) +
  facet_grid(level ~ year, scales = "free_x") +
  geom_hline(yintercept = 0, lty = 2) +
  geom_segment(aes(xend = chapter_simple), yend = 0) +
  geom_point() +
